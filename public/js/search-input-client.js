...
document.getElementById('delClientName').addEventListener('input', function () {
  ...
  // Search Input to Edit Client PIC
document.getElementById('editClientName').addEventListener('input', function () {
	const searchQuery = this.value.trim();
  
	// If the search query is empty, hide the client list and return
	if (searchQuery === '') {
	  document.getElementById('editClientNameList').innerHTML = '';
	  return;
	}
	...
	$.ajax({
	  ...
	  data: { keyword: searchQuery }, // Pass the search query as data
	  success: function (response) {
		displayEditClientName(response);
	  },
	  error: function (xhr, status, error) {
		console.error('Failed to fetch client names.');
	  },
  ...
  function displayEditClientName(response) {
	const clientNameList = document.getElementById('editClientNameList');
	clientNameList.innerHTML = '';
	...
	if (response.length > 0) {
	  response.forEach(function (client) {
		...
		option.className = 'clientNameOption ms-1';
		option.addEventListener('click', function () {
		  document.getElementById('editClientName').value = client.name; // Replace 'name' with the correct property from the response
		  clientNameList.innerHTML = '';
		});
		clientNameList.appendChild(option);
	  });
